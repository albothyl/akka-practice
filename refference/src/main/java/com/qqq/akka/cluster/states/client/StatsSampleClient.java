package com.qqq.akka.cluster.states.client;
...
import com.qqq.akka.cluster.states.StatsMessages.JobFailed;
import com.qqq.akka.cluster.states.StatsMessages.StatsJob;
import com.qqq.akka.cluster.states.StatsMessages.StatsResult;
...
public class StatsSampleClient extends AbstractActor {
  ...
  final Set<Address> nodes = new HashSet<>();
  ...
  public StatsSampleClient(String servicePath) {
    ...
    tickTask = getContext()
        .system()
        .scheduler()
        .schedule(interval, interval, self(), "tick", getContext().dispatcher(), null);
  }
